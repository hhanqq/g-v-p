...
func (server *Server) loadAlertsOverTimeSeries(ctx context.Context) ([][]any, error) {
...
func (server *Server) loadTopProblemObjects(ctx context.Context) ([]map[string]any, error) {
	rows, err := server.pool.Query(ctx, `
	...
	return topObjects, nil
}

func (server *Server) loadTopSymptomsAndPriorities(ctx context.Context) ([][]any, map[string]int64, error) {
	...
	rows.Close()
	return topSymptoms, priorities, nil
}

func (server *Server) loadSLABreaches(ctx context.Context) (map[string]int64, int64, error) {
	rows, err := server.pool.Query(ctx, `
	...
	return breaches, breachTotal, nil
}

func (server *Server) analyticsSummary(response http.ResponseWriter, request *http.Request, _ map[string]any) {
	ctx := request.Context()
	series, err := server.loadAlertsOverTimeSeries(ctx)
	if err != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
		return
	}
	topObjects, err := server.loadTopProblemObjects(ctx)
	if err != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
		return
	}
	topSymptoms, priorities, err := server.loadTopSymptomsAndPriorities(ctx)
	if err != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
		return
	}
	breaches, breachTotal, err := server.loadSLABreaches(ctx)
	if err != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
		return
	}
...
func (server *Server) createScenario(response http.ResponseWriter, request *http.Request, user map[string]any) {
	...
	var id int64
	err = tx.QueryRow(ctx, `INSERT INTO scenarios(name,description,graph_json,status,created_by,created_at,updated_at) VALUES($1,$2,$3,'draft',$4,$5,$5) RETURNING id`, strings.TrimSpace(*payload.Name), payload.Description, *payload.GraphJSON, actor, now).Scan(&id)
	if err == nil {
		_, err = tx.Exec(ctx, `
	...
	if err == nil {
		_, err = tx.Exec(ctx, `INSERT INTO audit_log(actor,action,target,created_at) VALUES($1,'create_scenario',$2,$3)`, actor, strings.TrimSpace(*payload.Name), now)
	}
...
func (server *Server) setScenarioStatus(response http.ResponseWriter, request *http.Request, user map[string]any, status string) {
	...
	var name, graphJSON string
	ctx := request.Context()
	if err := server.pool.QueryRow(ctx, `SELECT name,graph_json FROM scenarios WHERE id=$1`, id).Scan(&name, &graphJSON); errors.Is(err, pgx.ErrNoRows) {
		writeError(response, http.StatusNotFound, "Сценарий не найден")
		return
	} else if err != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
	...
	defer func() { _ = tx.Rollback(ctx) }()
	_, err = tx.Exec(ctx, `UPDATE scenarios SET status=$2,updated_at=$3 WHERE id=$1`, id, status, now)
	if err == nil {
		_, err = tx.Exec(ctx, `INSERT INTO audit_log(actor,action,target,created_at) VALUES($1,$2,$3,$4)`, actor, action, name, now)
	}
	if err != nil || tx.Commit(ctx) != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
		return
...
func (server *Server) createSLARule(response http.ResponseWriter, request *http.Request, user map[string]any) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	var id int64
	err = tx.QueryRow(ctx, `INSERT INTO sla_rules(name,priority,subsidiary,service_id,response_minutes,resolution_minutes,created_at) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id`, strings.TrimSpace(payload.Name), payload.Priority, payload.Subsidiary, payload.ServiceID, payload.ResponseMinutes, payload.ResolutionMinutes, now).Scan(&id)
	if err == nil {
		_, err = tx.Exec(ctx, `INSERT INTO audit_log(actor,action,target,created_at) VALUES($1,'create_sla_rule',$2,$3)`, actor, strings.TrimSpace(payload.Name), now)
	}
	if err != nil || tx.Commit(ctx) != nil {
		writeError(response, http.StatusServiceUnavailable, "database unavailable")
		return
