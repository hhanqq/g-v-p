...
<h2>POST /api/v1/ingest/raw</h2><form id="f"><label>source_system</label><input id="s" value="zabbix"><label>source_instance</label><input id="i" value="zbx-demo"><label>raw_body</label><textarea id="b">PROBLEM: demo</textarea><button>Отправить</button></form><pre id="o"></pre>
<script>f.onsubmit=async(e)=>{e.preventDefault();let r=await fetch('/api/v1/ingest/raw',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({source_system:s.value,source_instance:i.value,raw_body:b.value})});o.textContent=r.status+'\n'+await r.text()}</script></html>`
