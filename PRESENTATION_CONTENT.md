...
| 3 | Использование ИИ | 0.15 | **4/5** | 6 сценариев в отдельном модуле, работают на разноформатных сообщениях — это дословно планка «4 балла». До 5 не хватает одного ингредиента 5-балльной формулировки — честно называем это дальше |
...
**Взвешенный итог: 4.55 / 5 (≈91%)** — если пройти по этому файлу
слайд за слайдом и отработать Приложение А. Это честная, защитимая
цифра: она выше, чем если бы мы тихо промолчали про два реальных
пробела (умная маршрутизация по истории, выделенные ресурсы под ИИ), и
не рискует тем, что дотошный вопрос жюри вскроет нестыковку и
...
**Критерий 3. ИИ — честно 4/5, не 5/5.** Дословная планка 5 баллов:
«комплексный ИИ-агрегатор: нормализация + дедупликация + корреляция с
определением первопричины + **умная маршрутизация на основе истории**
+ саммаризация». У нас пять из шести перечисленных элементов плюс ещё
один сверху (RAG-разбор по запросу), но именно «умная маршрутизация на
основе истории» существовала только в старом Python-прототипе
(`suggest_subscription.py`) и не перенесена на Go-ядро при миграции —
реальный, не бумажный пробел. Дословная планка 4 баллов: «реализовано
более 3 сценариев, ИИ выделен в отдельный модуль, корректная работа на
разноформатных сообщениях от разных систем» — это именно то, что у нас
есть, построчно и без натяжек. Решение: на слайде говорим о 6
сценариях с гордостью, а недостающий описываем как «дорожную карту»,
а не прячем — жюри регулярно проверяет ровно такие списки построчно,
и обнаруженное несоответствие роняет доверие сильнее, чем честно
названный пробел.
...
**Слайд 12 — шесть сценариев на одном модуле.**

**Схема** — «колесо» из 6 секторов вокруг центрального блока Ollama
...
- **Честная дорожная карта** (нижний блок слайда, не прячем): «Умная
  маршрутизация на основе истории инцидентов — следующий шаг. Прототип
  существовал на предыдущей архитектуре, при миграции ядра на Go не
  перенесён — сознательный приоритет в пользу перечисленных выше шести
  работающих сценариев». Эта строка — то, что превращает 4/5 в
  по-настоящему честный, а не «недоделанный» результат в глазах жюри.
