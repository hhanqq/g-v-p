...
Дата подготовки: 2026-08-09, финальная сверка 2026-08-10. Источники
фактов: живой стенд `https://xn--80aebrvrg.xn--p1acf/`,
`alert-platform/ARCHITECTURE.md`, `INFRASTRUCTURE.md`, `SECURITY.md`,
`GO-MIGRATION.md`, `alert-platform/README.md` (инструкция для
проверяющего) и `alert-platform/COMPLIANCE_MATRIX.md` (построчное
соответствие каждому пункту 5-балльной планки с доказательствами —
техническое приложение к разделу 1 ниже), прямые запросы к продовой
БД и API в день подготовки, и сам файл `КЕЙС_студенты_ITCamp.pdf`
(везде, где ниже написано «кейс» без другой ссылки — это дословная
цитата или пересказ этого документа).
...
| 6 | Инфраструктура и масштабируемость | 0.10 | **4/5** | Пропускная способность, хранение/бэкапы и теперь резервирование PostgreSQL (реальная streaming-реплика, закрыто 2026-08-10) — закрыты; «обособленные ресурсы для ИИ-модуля» — честно не сделано, обоснование ниже |
...
| 8 | Презентация и обоснование требований | 0.10 | **5/5 (закрыто 2026-08-10)** | Материал на связный, непротиворечивый рассказ по всем разделам + сквозной аудит `README.md`/`COMPLIANCE_MATRIX.md`/`SECURITY.md`/кода/этого документа друг против друга — расхождений не найдено, единственные оставшиеся пробелы (экономика/ИБ-инфраструктура/`взял`-`не мой`-`следствие`) названы одинаково и честно во всех документах, ни в одном не выдаются за готовые |

**Взвешенный итог: ≈4.80 / 5 (≈96%)** — если пройти по этому файлу
...
**Критерий 6. Инфраструктура — было 4/5, частично закрыто 2026-08-10,
остаётся честно 4/5.** Планка 5 баллов требует три вещи: отказоустойчивость
(резервирование + недопущение потери событий), **обособленные ресурсы
для ИИ-модуля**, мониторинг/аналитика. Из трёх теперь закрыты две:
события не теряются при сбое (WORM + идемпотентность + двухфазная
обработка очереди — раздел 5 ниже), мониторинг есть (`/app`,
`INFRASTRUCTURE.md`), и **резервирование теперь тоже реальное** —
асинхронная streaming-реплика PostgreSQL, поднята без даунтайма
primary (живой `CREATE ROLE` + `pg_reload_conf()`, не restart),
измеренная задержка репликации 0,591мс, проверено сквозным тестом
(запись на primary видна на реплике по факту, не по теории).

Единственное, что осталось честно открытым: Ollama сегодня делит
CPU/RAM/GPU с остальным хостом — выделенных ресурсов под ИИ-модуль
нет. Это ровно то, что сам кейс называет следующим шагом бюджета
(«выделение двух серверов с GPU NVIDIA A30… увеличит смету на 400 тыс.
рублей») — то есть у нас есть готовое, кейсом же санкционированное
обоснование, почему это осознанный вырез пилота, а не недосмотр. Мы
сознательно не пытались закрыть это косметикой (например, докеризацией
Ollama без реальной второй GPU) — риск сломать все 7 живых ИИ-сценариев
ради пункта, у которого и так есть честное объяснение, того не стоил.
...
- Резервное копирование PostgreSQL — ежедневный cron, проверено
  вживую;
- **Резервирование PostgreSQL** — асинхронная streaming-реплика,
  поднята без единой секунды простоя primary; измеренная задержка
  репликации — 0,591мс; сквозной тест (запись через реальный `POST
  /api/v1/ingest/raw` на primary видна на реплике мгновенно) прошёл
  живьём 2026-08-10;
