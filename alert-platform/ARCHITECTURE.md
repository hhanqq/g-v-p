...
Go pipeline/planner/admin-console используют один локальный Ollama
endpoint; одинаковая политика отказа везде — раздел И5: сбой/таймаут ИИ
никогда не блокирует конвейер или экран, деградация до поведения без
ИИ-компонента.
...
| Умная маршрутизация на основе истории | Go admin-console (`internal/adminapi/subscription_suggestion.go`) | Сотруднику без подписок предлагается паттерн подписки, реально основанный на истории подписок коллег из тех же групп (`GROUP BY subsidiary,service_id,priority ORDER BY count DESC`) — ИИ только формулирует объяснение по готовым фактам, не выдумывает их |

Последний сценарий — единственный в списке, который работает не в
пайплайне обработки алертов, а в личном кабинете и на карточке
сотрудника: показывает, что архитектура допускает подключение ИИ к
любой части системы через один и тот же `OllamaClient`, а не только
к конвейеру.
