...
| Локальный ИИ без внешнего облака | Ollama остаётся внутри контура и вне критического пути доставки | Сохраняется |
...
- В Go gateway пока нет mTLS/HMAC, индивидуальных токенов источников,
  allow-list и rate limiting. Есть строгая JSON-валидация, лимит тела и
  WORM+queue до ответа, но защита интеграционного API — отдельный следующий
  этап, а не заявленная готовность.
